...
class BehaviorLogger:
    ...
    def log_code_edit(self, student_id: str, session_id: str, 
                     code_before: str, code_after: str, 
                     cursor_pos: Dict[str, int], current_task: str = None) -> None:
        """记录代码编辑事件"""
    ...
    def log_pause(self, student_id: str, session_id: str, 
                  duration: float, cursor_pos: Dict[str, int]) -> None:
        """记录思考停顿事件"""
        ...
        event = BehaviorEvent(
            ...
            cursor_position=cursor_pos,
            metadata={
                'pause_category': self._categorize_pause(duration)
            }
        )
        
        self.log_event(event)
    
    def log_error_event(self, student_id: str, session_id: str,
    ...
    def log_help_request(self, student_id: str, session_id: str,
                        help_query: str, ai_response: str = None) -> None:
        """记录求助事件"""
    ...
    def get_session_summary(self, session_id: str) -> Dict[str, Any]:
        ...
        avg_pause_duration = sum(e.duration or 0 for e in pause_events) / max(len(pause_events), 1)
        ...
        help_frequency = len(help_events) / max(session_duration / 60, 1)  # 每分钟求助次数
        
        return {
            'session_duration': session_duration,
            'total_events': len(events),
            'edit_frequency': total_edits / max(session_duration / 60, 1),  # 每分钟编辑次数
    ...
    def extract_learning_signals(self, student_id: str, 
        ...
        # 分析学习信号
        return {
            'cognitive_load_signals': self._extract_cognitive_load_signals(recent_events),
            ...
            'learning_preference_signals': self._extract_preference_signals(recent_events)
        }
    ...
    def _get_event_type_distribution(self, events: List[BehaviorEvent]) -> Dict[str, float]:
        """获取事件类型分布"""
        total = len(events)
        if total == 0:
            return {}
    ...
    def _extract_cognitive_load_signals(self, events: List[BehaviorEvent]) -> Dict[str, float]:
        """提取认知负荷信号"""
        edit_events = [e for e in events if e.event_type == EventType.CODE_EDIT]
        pause_events = [e for e in events if e.event_type == EventType.PAUSE_END]
        error_events = [e for e in events if e.event_type == EventType.ERROR_OCCUR]
        ...
        # 编辑频率（高频可能表示焦虑或试错）
        edit_frequency = len(edit_events) / max((events[-1].timestamp - events[0].timestamp) / 60, 1)
        
        # 平均停顿时长（长停顿可能表示困难）
        avg_pause = sum(e.duration or 0 for e in pause_events) / max(len(pause_events), 1)
    ...
    def _extract_confusion_signals(self, events: List[BehaviorEvent]) -> Dict[str, float]:
        """提取困惑信号"""
        edit_events = [e for e in events if e.event_type == EventType.CODE_EDIT]
        help_events = [e for e in events if e.event_type == EventType.HELP_REQUEST]
        error_events = [e for e in events if e.event_type == EventType.ERROR_OCCUR]
        ...
        if len(error_events) > 1:
            ...
            error_clustering = 1.0 if avg_error_interval < 30 else 0.0  # 错误间隔小于30秒
        ...
        if len(edit_lengths) > 1:
            ...
            variance = sum((x - avg_length) ** 2 for x in edit_lengths) / len(edit_lengths)
        ...
        return {
            ...
            'high_help_seeking': 1.0 if help_frequency > 2 else 0.0,  # 每分钟超过2次求助
        }
    
    def _extract_engagement_signals(self, events: List[BehaviorEvent]) -> Dict[str, float]:
        """提取参与度信号"""
        if not events:
            return {}
        
        session_duration = events[-1].timestamp - events[0].timestamp
        active_events = [e for e in events if e.event_type in [
            EventType.CODE_EDIT, EventType.CODE_EXECUTE, EventType.AI_CHAT
        ]]
        ...
        return {
            ...
            'sustained_engagement': 1.0 if activity_density > 5 else 0.0  # 每分钟超过5个活跃事件
        }
    
    def _extract_preference_signals(self, events: List[BehaviorEvent]) -> Dict[str, float]:
        """提取学习偏好信号"""
        help_events = [e for e in events if e.event_type == EventType.HELP_REQUEST]
        ...
        for event in help_events:
            ...
            if any(keyword in query.lower() for keyword in ["代码", "语法", "实现", "怎么写"]):
                code_related_queries += 1
            elif any(keyword in query.lower() for keyword in ["什么是", "为什么", "概念", "理解"]):
